...
var isArray = __webpack_require__(0),
    ...
    stringToPath = __webpack_require__(18),
...
function byKeyValue_(key, value) {
  return function (item) {
    if (!key || _typeof(item) !== 'object') {
      return false;
    }
...
function byObj_(obj) {
  if (!lodash_isPlainObject__WEBPACK_IMPORTED_MODULE_0___default()(obj)) return [];
...
function createMapByFn(fn) {
  return map_(fn);
}
function createMapByKeys(keys) {
  return map_(function (item) {
    return lodash_pick__WEBPACK_IMPORTED_MODULE_3___default()(item, keys);
  });
}
function createMapByKey(key) {
  return map_(byKey_(key));
}
function createFilterByFn(predicate) {
  return filter_(predicate);
}
function createFiltersByQueryObject(queryObject) {
  return byObj_(queryObject).map(filter_);
}
function createFilterByPair(key, value) {
  return filter_(byKeyValue_(key, value));
}
function createSortByFn(fn, direction) {
  return sort_(fn, direction);
}
function createSortByKey(key, direction) {
...
function query(collection, operations) {
  ...
  function build(operations) {
    if (lodash_isArray__WEBPACK_IMPORTED_MODULE_2___default()(operations)) {
  ...
  return {
     ...
     */
    where: function where(arg, value) {
      if (lodash_isPlainObject__WEBPACK_IMPORTED_MODULE_0___default()(arg)) {
        return build(createFiltersByQueryObject(arg));
      }

      if (lodash_isFunction__WEBPACK_IMPORTED_MODULE_1___default()(arg)) {
     ...
     */
    orderBy: function orderBy(arg, direction) {
      if (lodash_isFunction__WEBPACK_IMPORTED_MODULE_1___default()(arg)) {
    ...
    select: function select(arg) {
      ...
      if (lodash_isArray__WEBPACK_IMPORTED_MODULE_2___default()(arg)) {
...
var baseGet = __webpack_require__(15),
    baseSet = __webpack_require__(21),
...
var memoizeCapped = __webpack_require__(19);
...
var baseHasIn = __webpack_require__(28),
